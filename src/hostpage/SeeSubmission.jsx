  ...
  const { data: submission = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const result = await axiosPublic.get(
        `/host/contest/title/${user?.email}?contest=${title}`
      );
      return result.data;
    },
  });
                  ...
                  {submission.map(item => (
                    <tr key={item._id}>
                      ...
                      {item.status === 'pending' && (
                        <td>
                          <button
                            onClick={() => winHandler(item._id)}
                            className="btn bg-[#0ecdb9]"
                          >
                            Win
                          </button>
                        </td>
                      ...
                      {item.status === 'winner' && (
                        <td className="font-extrabold">winner</td>
                      )}
                      {item.status === 'Unsuccess' && (
                        <td className="font-extrabold">Un-success</td>
                      )}
