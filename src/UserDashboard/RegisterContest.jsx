...
const RegisterContest = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const register = await axiosPublic.get(
        `/getRegisterContest/${user?.email}`
      );
  ...
  return (
    <div>
      <div
        ...
        style={{
          backgroundImage:
            'url(https://egamlio.vercel.app/images/top-coches-bg.png)',
        }}
      >
              ...
              <tr key={item?._id} className="space-x-4">
                ...
                <td>
                  ...
                  <Link to={`/payment/${item?._id}`}>
                    <button className="btn bg-[#0ecdb9] border-none rounded-2xl  text-white">
                      Pay{' '}
                    </button>
                  </Link>
                </td>
